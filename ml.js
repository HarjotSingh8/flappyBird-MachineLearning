...
function nextGeneration() {
  ...
  for (let i = parseInt(birds.length * percentToKeep); i < birds.length; i++) {
    ...
    birds[i].ml.copyAndMutate(
      birds[parseInt(i % (birds.length * percentToKeep))].ml
    );
  }
...
function sortByFitness() {
  ...
  for (let i = 0; i < birds.length; i++) {
    for (let j = i; j < birds.length; j++) {
      if (birds[i].distance < birds[j].distance) {
        tempml.copy(birds[i].ml);
        birds[i].ml.copy(birds[j].ml);
        birds[j].ml.copy(tempml);

        let temp = birds[i].distance;
        birds[i].distance = birds[j].distance;
...
class MachineLearning {
  ...
  copy(source) {
    ...
    //this.resetFitness();
    this.mutateWeightsAndBiases(source, 0);
  }
